...
- **A1b display layer — date formatting sweep incomplete.** PR #34 converted only EmployeeHome's My Info card to DD/MM/YY (`formatDateMXLong`). ~25 remaining date display sites still use `toLocaleDateString("en-US", ...)`: EmpleadoPerfil compliance/note/doc/incident/policy timestamps, EmployeeHome attendance/time-off/doc approval labels, TLDashboard heatmap + coaching notes, TimeOff request ranges, Policies publish/ack dates, Historial biweekly close, Timeclock, Performance, ClockOutEODDialog, EODForm. Mechanical conversion — one follow-up PR. Added during A1b display layer (2026-04-21).

- **A1b display layer — TL data layer + greeting missing work_name.** `useTeamLead.ts` (26 occurrences of `m.full_name`) doesn't select `work_name` from `employees_no_pay` despite the view now exposing it. `TeamLeadHome.tsx:351, 374` greets TLs with their legal first name (pseudonym leak). Also `EmpleadoPerfil.tsx:300` inlines the display-name logic instead of calling `getDisplayName()`, and `Policies.tsx:428` shows uploader's legal full_name. Bundle these as one follow-up PR. Added during A1b display layer (2026-04-21).
