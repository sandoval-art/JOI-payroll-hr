...
5. **B2/B3 — Carta + acta request flow with split-view editor + template engine + signed-scan upload.** Phase 1 shipped (schema, PR #42). Phase 2 shipped (TL request-filing form + requests card on agent profile, PR #43). Phases 3–5 queued: HR queue/status → split-view editor + PDF + doc-ref generator → signed-scan upload + agent view. Phase 4 editor is gated on the break-times file (needed for `horario_snapshot`).
