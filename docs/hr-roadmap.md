...
- ~~**Harden RLS on employees table for sensitive fields (curp, rfc, bank_clabe).**~~ ✅ **DONE 2026-04-21 (PR #32).** `employees_no_pay` view rebuilt as `security_invoker=off` with role-scoped WHERE clause; `tl_select_team_employees` policy dropped from base table. TLs now have zero direct SELECT on `employees`; must read safe columns via `employees_no_pay` view. UPDATE stays leadership-only as before.
