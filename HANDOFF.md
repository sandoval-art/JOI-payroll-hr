...
- **Feature F resignation packet** (Phase F1) — `resignation_packets` table + LFT calculation engine (`src/lib/lftCalculations.ts`). Aguinaldo proporcional, vacaciones correspondientes (Art. 76 post-2023), prima vacacional, total + number-to-Spanish-words. 28 unit tests including Edgar Barron template sanity check. Schema only, no UI yet (Phase F2).
