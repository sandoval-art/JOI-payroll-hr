...
- **Multi-tenancy Phase 1–3b** — **COMPLETE** (PRs #80–#83, 2026-04-27). Shared-DB org isolation across the entire schema. 8 tables carry an `organization_id NOT NULL` FK directly (`organizations`, `user_profiles`, `clients`, `campaigns`, `employees`, `payroll_periods`, `departments`, `policy_documents`, `company_holidays`). All remaining 22 leaf tables are org-scoped via subquery in RLS. Every RLS policy checks `my_org_id()`. An agent from Org A can never read, write, or infer data belonging to Org B. Phase 4 (make SECURITY DEFINER helpers org-aware) and Phase 5 (org provisioning UI) are still pending.
